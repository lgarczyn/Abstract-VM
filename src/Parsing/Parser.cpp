...
Parser::Parser( const Parser& cpy ) = default;
Parser::~Parser() = default;
Parser& Parser::operator=( const Parser& cpy ) = default;
...
Operation Parser::getOperation( OperationToken token )
{
	...
	if ( takes_value && token.has_value == false )
		throw std::logic_error(
			std::string( "No value provided for operation: " ) + token.operator_name );

	if ( takes_value == false && token.has_value )
		throw std::logic_error(
			std::string( "Useless value provided for operation: " ) + token.operator_name );
