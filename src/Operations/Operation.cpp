...
Operation::Operation() : _type(), _operand() {}
Operation::Operation( eOperationType type, const IOperand* operand ) :
	_type( type ), _operand( operand )
{
}
Operation::Operation( const Operation& cpy ) : _type( cpy._type )
{
	if ( cpy._operand )
		this->_operand = cpy._operand->clone();
	else
		this->_operand = NULL;
}
...
void Operation::op_push( Stack& stack ) { stack.push_back( _operand ); }
...
void Operation::op_assert( Stack& stack )
{
	...
	//{
	//	std::stringstream ss;
	//	ss << "Failed assert: " << this->_operand->getTypeName() << " != " <<
	// stack.back()->getTypeName(); 	throw std::logic_error(ss.str());
	//}
